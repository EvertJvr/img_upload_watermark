...
file = 'progress'
PyPDF4.PdfFileReader(f'./working files/{file}.pdf')


def put_watermark(input_pdf, output_pdf, watermark):
    # reads the watermark pdf file through
    # PdfFileReader
    watermark_instance = PdfFileReader(watermark)  # PDF file
...
if __name__ == "__main__":
    put_watermark(
        input_pdf=f'./working files/{file}.pdf',  # the original pdf
        output_pdf=f'./working files/{file}(wm).pdf',  # the modified pdf with watermark
        watermark='./assets/NotForTender.pdf'  # the watermark to be provided
    )
